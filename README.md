...
想親手轉一次那張紙盤？我們照台版手冊掃描件重繪了一個**可玩的線上轉盤**：[`docs/mixnmojo/`](docs/mixnmojo/index.html)（雙層可旋轉、15 個位置即時顯示 7 道配方的密碼，還附一個模擬遊戲考題的挑戰模式）。把本 repo 的 GitHub Pages 打開後，它就是一個網頁；本機也可直接用瀏覽器開 `docs/mixnmojo/index.html`。
