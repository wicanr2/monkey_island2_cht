...
## 5.9 #2 防拷自動略過（2026-07-16，已實作 patch）

- **確認 DOS 版防拷不被 ScummVM 略過**（只 Mac 有 boot-script patch），必須輸入正確答案（實測錯答案被擋）。
- **反組譯定位驗證點**：`descumm`（scummvm-tools）反組譯 room 108 腳本 → `SCRP_0130`：`[0250] Var[490]=正確答案；[0255] if (Var[518]≠Var[490]) 錯誤(試3次→systemOps(3)退出/重試)`。Var[518]=玩家輸入。
- **hook**（`patches/0001-mi2-dos-copyprotection-bypass.patch`，script.cpp writeVar，8 行）：DOS MI2 + room 108 + `copy_protection=false` 時，寫 Var[490] 同步鏡射 Var[518]=同值 → 比對永遠相等 → **輸入任意兩數字即通過**（不必查密碼轉盤）。
- 驗證：正常開機、配方畫面輸入「5555」(錯) → **直接進遊戲開場**（Guybrush 吊繩深坑，旁白「哎呀哎呀哎呀。」，`docs/screenshots/cp_bypass_intro.png`）。
- 密碼轉盤完整答案表（公開資料，Steam 等可查）：`docs/copyprotection-answer.md`。
- **限制**：目前是「任意答案即過」，玩家仍需輸入兩個數字按 Enter；若要「完全零互動略過畫面」需再 patch boot/room108 進場（可選 follow-up）。
